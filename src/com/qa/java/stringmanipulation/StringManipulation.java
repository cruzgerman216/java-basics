...
public class StringManipulation {
	...
	public static boolean findMessage(String message, String longmessage) {
		...
		for(int i = 0; i < longmessage.length(); i++) {
			if(i + message.length() < longmessage.length() && longmessage.substring(i, i + message.length()).equals(message)) {
				System.out.println(longmessage.substring(i, i + message.length()));
				flag = true;
			}
		}
		return flag;
